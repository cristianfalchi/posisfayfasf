...
function handleUpdateData() {
   console.log("estoy en hndleUpdateData");
...
function getFile() {
   ...
   fetch(`${loc[0]}descargar-secuencia?nroSec=${nroSec}`)
...
document.getElementById("check_input").addEventListener('change', (event) => {
   const input_hora = document.getElementById("hora");
   console.log(input_hora);
   if (!event.target.checked) {
      input_hora.setAttribute("disabled", '');
   } else {
      input_hora.removeAttribute("disabled");
   }
})

function desprogramar(event) {
